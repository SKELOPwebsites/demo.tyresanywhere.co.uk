<template>
    <div>
        <h2 class="font-montserrat text-xl font-semibold mb-8">Recent blog posts</h2>
        ...
        <div class="grid xl:grid-cols-1 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-12">
            <template v-for="(post, index) in filteredBlogPosts">
                <Link :href="post.href" class="group hover:cursor-pointer">
                    <img :src="post.img" :alt="post.alt" class="mb-4 w-full h-[200px] object-cover" />
                    <div class="space-y-3">
                        <div class="flex space-x-1 text-gray-400 font-montserrat">
                            <span class="text-sm">{{ post.author }}</span>
                            <span class="text-sm">&bull;</span>
                            <span class="text-sm">{{ formatDate(post.date) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <h3 class="font-montserrat font-semibold md:text-xl text-base group-hover:underline">{{ post.title }}</h3>
                        ...
                        <p class="text-slate-500 line-clamp-3 md:text-base text-sm">{{ post.description }}</p>
                    </div>
                </Link>
            </template>
        </div>
